...
function areSocketsInSameRoom(socket: Socket, targetSocketId: string, io: Server): boolean {
    const targetSocket = io.sockets.sockets.get(targetSocketId);
    if (!targetSocket) return false;

    for (const room of socket.rooms) {
        if (room === socket.id) continue;
        if (targetSocket.rooms.has(room)) return true;
    }
    return false;
}
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('offer', (payload: { targetSocketId: string, offer: any }) => {
            try {
                ...
                if (!areSocketsInSameRoom(socket, payload.targetSocketId, io)) return;
